...
void test_target_teams_distribute__parallel__simd() {
  ...
  #pragma omp target teams distribute map(tofrom: counter_N0) reduction(+: counter_N0) collapse(2)
  ...
  for (int i1 = 0 ; i1 < N1 ; i1++ )
  {
    ...
    #pragma omp parallel reduction(+: counter_parallel)
    {
      ...
      counter_parallel = counter_parallel + complex<double> { counter_N2.real() / omp_get_num_threads() };
    }
