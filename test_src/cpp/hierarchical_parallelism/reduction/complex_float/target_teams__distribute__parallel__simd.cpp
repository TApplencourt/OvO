...
void test_target_teams__distribute__parallel__simd.cpp(){
...
if ( !almost_equal(counter,complex<float> { L*M }, 10)  ) {
    ...
    throw std::runtime_error( "target_teams__distribute__parallel__simd.cpp give incorect value when offloaded");
}

}
int main()
{
    test_target_teams__distribute__parallel__simd.cpp();
}
