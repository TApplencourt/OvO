...
void test_target__parallel__loop__simd.cpp(){
...
if ( !almost_equal(counter,complex<float> { L*M }, 10)  ) {
    ...
    throw std::runtime_error( "target__parallel__loop__simd.cpp give incorect value when offloaded");
}

}
int main()
{
    test_target__parallel__loop__simd.cpp();
}
