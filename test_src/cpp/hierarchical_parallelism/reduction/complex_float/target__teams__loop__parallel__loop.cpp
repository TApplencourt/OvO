...
void test_target__teams__loop__parallel__loop.cpp(){
...
if ( !almost_equal(counter,complex<float> { L*M }, 10)  ) {
    ...
    throw std::runtime_error( "target__teams__loop__parallel__loop.cpp give incorect value when offloaded");
}

}
int main()
{
    test_target__teams__loop__parallel__loop.cpp();
}
