...
void test_target__teams_distribute_simd.cpp(){
...
#pragma omp teams distribute simd  reduction(  ComplexReduction  :counter)  
...
if ( !almost_equal(counter,complex<float> { L }, 10)  ) {
    ...
    throw std::runtime_error( "target__teams_distribute_simd.cpp give incorect value when offloaded");
}

}
int main()
{
    test_target__teams_distribute_simd.cpp();
}
