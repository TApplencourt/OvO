...
void test_target__teams__distribute__parallel__loop__simd.cpp(){
...
if ( !almost_equal(counter,complex<double> { L*M*N }, 10)  ) {
    ...
    throw std::runtime_error( "target__teams__distribute__parallel__loop__simd.cpp give incorect value when offloaded");
}

}
int main()
{
    test_target__teams__distribute__parallel__loop__simd.cpp();
}
