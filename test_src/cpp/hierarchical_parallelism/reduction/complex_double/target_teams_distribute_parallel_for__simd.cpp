...
void test_target_teams_distribute_parallel_for__simd.cpp(){
...
#pragma omp target teams distribute parallel for  reduction(  ComplexReduction  :counter)   map(tofrom:counter) 
...
if ( !almost_equal(counter,complex<double> { L*M }, 10)  ) {
    ...
    throw std::runtime_error( "target_teams_distribute_parallel_for__simd.cpp give incorect value when offloaded");
}

}
int main()
{
    test_target_teams_distribute_parallel_for__simd.cpp();
}
