...
void test_target_teams_loop__parallel.cpp(){
...
if ( !almost_equal(counter,complex<double> { L }, 10)  ) {
    ...
    throw std::runtime_error( "target_teams_loop__parallel.cpp give incorect value when offloaded");
}

}
int main()
{
    test_target_teams_loop__parallel.cpp();
}
