...
void test_target__teams__parallel_loop__simd.cpp(){
...
if ( !almost_equal(counter,complex<double> { L*M }, 10)  ) {
    ...
    throw std::runtime_error( "target__teams__parallel_loop__simd.cpp give incorect value when offloaded");
}

}
int main()
{
    test_target__teams__parallel_loop__simd.cpp();
}
