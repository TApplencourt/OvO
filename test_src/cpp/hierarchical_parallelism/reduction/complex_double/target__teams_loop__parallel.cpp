...
void test_target__teams_loop__parallel.cpp(){
...
if ( !almost_equal(counter,complex<double> { L }, 10)  ) {
    ...
    throw std::runtime_error( "target__teams_loop__parallel.cpp give incorect value when offloaded");
}

}
int main()
{
    test_target__teams_loop__parallel.cpp();
}
