...
void test_target__teams_loop__parallel_loop.cpp(){
...
if ( !almost_equal(counter,complex<double> { L*M }, 10)  ) {
    ...
    throw std::runtime_error( "target__teams_loop__parallel_loop.cpp give incorect value when offloaded");
}

}
int main()
{
    test_target__teams_loop__parallel_loop.cpp();
}
