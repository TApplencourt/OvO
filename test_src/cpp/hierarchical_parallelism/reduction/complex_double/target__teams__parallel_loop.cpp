...
void test_target__teams__parallel_loop.cpp(){
...
if ( !almost_equal(counter,complex<double> { L }, 10)  ) {
    ...
    throw std::runtime_error( "target__teams__parallel_loop.cpp give incorect value when offloaded");
}

}
int main()
{
    test_target__teams__parallel_loop.cpp();
}
