...
void test_target_teams_distribute_simd.cpp(){
  // Input and Outputs
...
#pragma omp target teams distribute simd   map(from: pA[0:L]) map(to: pB[0:L]) 
...
for (int i = 0 ;  i < size ; i++) {
    if ( !almost_equal(A[i],B[i],1) ) {
        ...
        throw std::runtime_error( "target_teams_distribute_simd.cpp give incorect value when offloaded");
    }
}
...
int main()
{
    test_target_teams_distribute_simd.cpp();
}