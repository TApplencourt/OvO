...
void test_target__teams__distribute__parallel__loop.cpp(){
  // Input and Outputs
...
for (int i = 0 ;  i < size ; i++) {
    if ( !almost_equal(A[i],B[i],1) ) {
        ...
        throw std::runtime_error( "target__teams__distribute__parallel__loop.cpp give incorect value when offloaded");
    }
}
...
int main()
{
    test_target__teams__distribute__parallel__loop.cpp();
}