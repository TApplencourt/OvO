...
void test_cyl_neumannl(){
   ...
   out2_host = cyl_neumannl( in0, in1, &out2_host);

   #pragma omp target map(from: out2_gpu )
   {
   out2_gpu = cyl_neumannl( in0, in1, &out2_gpu);
   }

   
   if ( !almost_equal(out2_host,out2_gpu,1) ) {
        std::cerr << "Host: " << out2_host << " GPU: " << out2_gpu << std::endl;
        throw std::runtime_error( "cyl_neumannl give incorect value when offloaded");
    }
