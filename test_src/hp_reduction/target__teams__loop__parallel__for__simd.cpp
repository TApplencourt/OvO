...
template<class T>
void test_target__teams__loop__parallel__for__simd(){
...
if ( !almost_equal(counter,T{ L*M*N }, 1)  ) {
    ...
    throw std::runtime_error( "target__teams__loop__parallel__for__simd give incorect value when offloaded");
}

}
int main()
{
    test_target__teams__loop__parallel__for__simd<double>();
}
