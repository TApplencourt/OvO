...
template<class T>
void test_target__teams__parallel_loop__simd(){
...
if ( !almost_equal(counter,T{ L*M }, 1)  ) {
    ...
    throw std::runtime_error( "target__teams__parallel_loop__simd give incorect value when offloaded");
}

}
int main()
{
    test_target__teams__parallel_loop__simd<double>();
}
