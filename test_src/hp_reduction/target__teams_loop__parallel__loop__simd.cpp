...
if ( !almost_equal(counter,T{ L*M*N }, 1)  ) {
    ...
    throw std::runtime_error( "target__teams_loop__parallel__loop__simd give incorect value when offloaded");
}

}
int main()
{
    test_target__teams_loop__parallel__loop__simd<double>();
}
