...
printf -- "exit %d\n" "$ret"; exit "$ret"; }; set -e; trimmed_doc=${DOC:0:564}
usage=${DOC:36:194}; digest=e42f2; shorts=('' '' '' '' '')
longs=(--v5 --no_long_double --no_loop --failure --pass); argcounts=(0 0 0 0 0)
node_0(){ switch __v5 0; }; node_1(){ switch __no_long_double 1; }; node_2(){
switch __no_loop 2; }; node_3(){ switch __failure 3; }; node_4(){
switch __pass 4; }; node_5(){ value _test_folder_ a true; }; node_6(){
value _result_folder_ a true; }; node_7(){ _command gen; }; node_8(){
_command run; }; node_9(){ _command display; }; node_10(){ _command clean; }
node_11(){ optional 0; }; node_12(){ required 7 11; }; node_13(){ oneormore 5; }
node_14(){ optional 13; }; node_15(){ optional 1; }; node_16(){ optional 2; }
node_17(){ required 8 14 15 16; }; node_18(){ either 3 4; }; node_19(){
...
[[ -n $1 ]] && printf "%s\n" "$1" >&2; printf "%s\n" "${DOC:36:194}" >&2; exit 1
}'; unset var___v5 var___no_long_double var___no_loop var___failure var___pass \
var__test_folder_ var__result_folder_ var_gen var_run var_display var_clean
parse 25 "$@"; local prefix=${DOCOPT_PREFIX:-''}; local docopt_decl=1
[[ $BASH_VERSION =~ ^4.3 ]] && docopt_decl=2; unset "${prefix}__v5" \
"${prefix}__no_long_double" "${prefix}__no_loop" "${prefix}__failure" \
...
declare -p "${prefix}__v5" "${prefix}__no_long_double" "${prefix}__no_loop" \
