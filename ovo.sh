...
Usage:
  ovo.sh gen
  ovo.sh run [<test_folder>...] [--no_long_double] [--no_loop] [--no_omp_call]
...
printf -- "exit %d\n" "$ret"; exit "$ret"; }; set -e; trimmed_doc=${DOC:0:239}
usage=${DOC:36:203}; digest=69fb6; shorts=('' '' '' '' '')
longs=(--no_long_double --no_loop --no_omp_call --failure --pass)
argcounts=(0 0 0 0 0); node_0(){ switch __no_long_double 0; }; node_1(){
switch __no_loop 1; }; node_2(){ switch __no_omp_call 2; }; node_3(){
...
optional 1; }; node_16(){ optional 2; }; node_17(){ required 8 13 14 15 16; }
node_18(){ either 3 4; }; node_19(){ optional 18; }; node_20(){ oneormore 6; }
node_21(){ optional 20; }; node_22(){ required 9 19 14 15 21; }; node_23(){
required 10; }; node_24(){ either 11 17 22 23; }; node_25(){ required 24; }
cat <<<' docopt_exit() { [[ -n $1 ]] && printf "%s\n" "$1" >&2
printf "%s\n" "${DOC:36:203}" >&2; exit 1; }'; unset var___no_long_double \
var___no_loop var___no_omp_call var___failure var___pass var__test_folder_ \
...
unset "${prefix}__no_long_double" "${prefix}__no_loop" \
"${prefix}__no_omp_call" "${prefix}__failure" "${prefix}__pass" \
...
eval "${prefix}"'__no_omp_call=${var___no_omp_call:-false}'
